...
def ingest_material(
    ...
    # 1. Generate the card (pure transformation, no persistence)
    card = generate_card(
    ...
    # 2. Save document to knowledge base (with dedup)
    doc_id = save_document(
        content=knowledge_seed,
        title=card.get("source_grounding", {}).get("source_title", "Untitled"),
        source_kind=card.get("source_grounding", {}).get("source_kind", ""),
        source_reference=source,
    )

    # 3. Chunk and index the document
    chunk_document(doc_id, knowledge_seed)

    # 4. Link card to document
